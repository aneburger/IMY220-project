...
const GlobalFeed = ({ filter, sort }) => {
    const [activities, setActivities] = useState([]);
    const navigate = useNavigate();

    useEffect(() => {
        fetch("http://localhost:3000/api/activity/global")
            .then(res => res.json())
            .then(data => {
    ...
    return (
        <div id="GlobalE">
            <section id="globalGrid">
                {filteredActivities.length === 0 ? (
                    <p id="noGlobalActDisplay">No activity to display yet. <span>(Activity of all users will display here.)</span></p>
                ):(
                filteredActivities.map(act => (
                    <div className="actCard" key={act._id}>
                        ...
                        <Link to={`/profile/${act.user}`}><p className="user">{act.user}</p></Link>
